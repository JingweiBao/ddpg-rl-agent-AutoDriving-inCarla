...
class Stage:
    ...
    def run(self, epochs: int, copy_weights=True, epoch_offset=0) -> 'Stage':
        assert epochs > 0
        self.init()
...
def evaluate(mode: str, town: str, seeds: list, traffic: str, steps=512, trials=50, weights='stage-s5'):
    def make_stage(weather):
        ...
        return stage_s1(episodes=1, timesteps=steps, batch_size=1, town=town, seed_regularization=True,
                        stage_name=weights,
                        weather=weather, aug_intensity=0.0, repeat_action=1, traffic=traffic, log_mode=None)
