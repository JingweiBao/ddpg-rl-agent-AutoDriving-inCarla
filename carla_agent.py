...
class CARLAgent(DDPGAgent):
    ...
    def policy_objective_ddpg(self, batch):
        states, actions, rewards, next_states, done = batch

        # Critic network predicts Q-values
        q_values = self.critic_predict(states, actions)

        # Actor network predicts actions
        predicted_actions = self.actor_predict(states)

        # Compute target Q-values using target critic network
        target_q_values = self.target_critic_predict(next_states, self.target_actor_predict(next_states))
        target_q_values = rewards + self.gamma * (1 - done) * target_q_values

        # Compute critic loss (Mean Squared Error)
        critic_loss = tf.reduce_mean(tf.square(q_values - target_q_values))

        # Actor loss - maximize Q-value
        actor_loss = -tf.reduce_mean(self.critic_predict(states, predicted_actions))
    ...
    def critic_objective_(self, batch):
        states, actions, rewards, next_states, done = batch
        
        # Compute target Q-values using target critic network
        target_next_actions = self.target_actor_predict(next_states)
        target_q_values = self.target_critic_predict(next_states, target_next_actions)
        target_q_values = rewards + self.gamma * (1 - done) * target_q_values
        
        # Predicted Q-values by the critic network
        q_values = self.critic_predict(states, actions)
