...
from rl.networks.networks import DDPGNetwork
...
class DDPGAgent(Agent):
    ...
    def __init__(self, *args, policy_lr: Union[float, LearningRateSchedule, DynamicParameter] = 1e-3, gamma=0.99,
                 ...
                 network: Union[dict, DDPGNetwork] = None, update_frequency=1, polyak=1.0, repeat_action=1,
        ...
        if isinstance(network, dict):
            network_class = network.pop('network', DDPGNetwork)

            if network_class is DDPGNetwork:
            ...
            self.network = DDPGNetwork(agent=self, policy={}, value={})
    ...
    def collect(self, episodes: int, timesteps: int, render=True, record_threshold=0.0, seeds=None, close=True):
        ...
        for episode in range(1, episodes + 1):
            ...
            self.reset()
            episode_reward = 0.0
            memory = DDPGMemory(state_spec=self.state_spec, num_actions=self.num_actions)
    ...
    def get_memory(self):
        ...
        return DDPGMemory(state_spec=self.state_spec, num_actions=self.num_actions)
...
class DDPGMemory:
    """Recent memory used in DDPGAgent"""
